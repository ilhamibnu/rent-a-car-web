...
<script>
    ...
    var options = {
        ...
        , series: [{
                name: "Total Harga"
                , data: dataSeries.map(item => item.total_harga)
            }
            , {
                name: "Jumlah Transaksi"
                , data: dataSeries.map(item => item.total_transaksi)
            }
        ]
        , title: {
            text: "Laporan Transaksi Mingguan"
            , align: "left"
        }
        , subtitle: {
            text: "Total Harga dan Jumlah Transaksi per Hari"
            , align: "left"
        }
        , labels: dataSeries.map(item => getDayName(item.date, 'id-ID'))
        , xaxis: {
            type: "category"
        }
        , yaxis: [{
    ...
    var chart = new ApexCharts(document.querySelector("#basic-apex-2"), options);
